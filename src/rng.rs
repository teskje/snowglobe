...
use libc::{CCRNGStatus, RTLD_NEXT, c_int, kCCSuccess, size_t};
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn CCRandomGenerateBytes(bytes: *mut c_void, size: size_t) -> CCRNGStatus {
    RNG.with_borrow_mut(|rng| {
        let Some(rng) = rng else {
...
unsafe extern "C" fn real_CCRandomGenerateBytes(bytes: *mut c_void, size: size_t) -> CCRNGStatus {
    static SYM: OnceLock<unsafe extern "C" fn(*mut c_void, size_t) -> c_int> = OnceLock::new();
